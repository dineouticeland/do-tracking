...
const SINGLE_ITEM_EVENTS = new Set([
    'Takeaway Item Selected',
    'Takeaway Item Viewed',
    'Takeaway Item Added',
    'Takeaway Item Removed',
...
function validateTakeawayItem(eventName, item, index) {
    ...
    for (const field of ['item_category', 'item_variant', 'item_list_id', 'item_list_name']) {
        if (item[field] !== undefined) {
            assertNonEmptyString(eventName, `items[${index}].${field}`, item[field]);
        }
    }
}
function takeawayExtraFields(eventName) {
    switch (eventName) {
...
function validateOrderId(eventName, value) {
...
function validateTakeawayPayload(eventName, properties) {
    assertObject(eventName, properties);
    const extraFields = takeawayExtraFields(eventName);
    const baseFields = (eventName === 'Takeaway Promo Rejected'
    ...
    for (const field of ['restaurant_name', 'cart_id']) {
        if (properties[field] !== undefined) {
            assertNonEmptyString(eventName, field, properties[field]);
        }
    }
    for (const field of ['tax', 'shipping']) {
        if (properties[field] !== undefined) {
    ...
    switch (eventName) {
        ...
        case 'Takeaway Gift Card Rejected':
            ...
            break;
        case 'Takeaway Payment Failed':
            if (!PAYMENT_FAILURE_CODES.has(properties.failure_code)) {
    ...
    if (properties.reason_code !== undefined) {
        if (!REJECTION_REASON_CODES.has(properties.reason_code)) {
...
function validateReservationCompleted(properties) {
...
function validateReservationDeposit(properties) {
...
export function validateEventPayload(eventName, properties) {
...
export function isTakeawayEvent(eventName) {
    return TAKEAWAY_EVENTS.has(eventName);
}
function copyDefined(properties) {
    if (!properties)
        return {};
    const copied = {};
...
function pickDefined(properties, fields) {
    const picked = {};
    for (const field of fields) {
        if (properties[field] !== undefined)
            picked[field] = properties[field];
    }
    return picked;
}
function normalizeItems(items) {
    return items.map((item) => pickDefined(item, TAKEAWAY_ITEM_FIELDS));
}
function normalizeCustomEvent(eventName, properties) {
    ...
    return { eventName, properties: copied, mapping: mapEventName(eventName) };
}
function takeawayProperties(eventName, properties) {
    ...
    if (eventName === 'Takeaway Promo Rejected'
        || eventName === 'Takeaway Gift Card Applied'
        || eventName === 'Takeaway Gift Card Rejected') {
...
function reservationLeadProperties(properties) {
    return pickDefined(properties, ['restaurant_id', 'restaurant_name', 'reservation_id', 'payment_required']);
}
function reservationDepositProperties(properties) {
    const value = properties.value;
    return Object.assign(Object.assign({}, pickDefined(properties, ['restaurant_id', 'restaurant_name', 'reservation_id', 'currency', 'value'])), { transaction_id: properties.payment_transaction_id, items: [{
...
function ga4Properties(eventName, properties) {
    if (isTakeawayEvent(eventName))
        return takeawayProperties(eventName, properties);
    if (eventName === 'Reservation Completed')
        return reservationLeadProperties(properties);
    if (eventName === 'Reservation Deposit Paid')
        return reservationDepositProperties(properties);
    return copyDefined(properties);
}
function mixpanelProperties(eventName, properties) {
    if (isTakeawayEvent(eventName)) {
...
function conversionFor(eventName, properties) {
    if (eventName === 'Takeaway Order Completed') {
...
function metaCommerceProperties(eventName, properties) {
    const items = normalizeItems(properties.items);
    const result = Object.assign({ content_type: 'product', content_ids: items.map((item) => item.item_id), contents: items.map((item) => ({
            id: item.item_id,
            quantity: item.quantity,
            item_price: item.price,
        })), num_items: items.reduce((total, item) => total + item.quantity, 0) }, pickDefined(properties, [
        'value',
        'currency',
        'restaurant_id',
        'restaurant_name',
    ...
    if (items.length === 1) {
        ...
        if (items[0].item_category !== undefined)
            result.content_category = items[0].item_category;
    }
    if (eventName === 'Takeaway Promo Rejected'
        || eventName === 'Takeaway Gift Card Applied'
        || eventName === 'Takeaway Gift Card Rejected') {
...
function metaReservationDepositProperties(properties) {
    return Object.assign({ content_type: 'product', content_ids: ['reservation_deposit'], contents: [{ id: 'reservation_deposit', quantity: 1, item_price: properties.value }], num_items: 1 }, pickDefined(properties, ['value', 'currency', 'restaurant_id', 'restaurant_name', 'reservation_id']));
}
/** Transform a descriptive event into GA4's event/parameter contract. */
export function toGA4Event(eventName, properties) {
...
export function toGTMEvent(eventName, properties) {
    ...
    return Object.assign(Object.assign({}, ga4.params), { event: ga4.eventName });
}
/** Transform a descriptive event into a Meta Pixel event, or null when intentionally unsupported. */
export function toMetaEvent(eventName, properties) {
    validateEventPayload(eventName, properties);
    const normalized = normalizeCustomEvent(eventName, properties);
    if (!normalized.mapping.fb)
        return null;
    let params;
    ...
    return Object.assign({ eventName: normalized.mapping.fb, custom: normalized.mapping.fbCustom, params }, (conversion ? { eventID: conversion.eventId } : {}));
}
/** Keep Mixpanel's readable event names while applying the same privacy-safe payload normalization. */
export function toMixpanelEvent(eventName, properties) {
    validateEventPayload(eventName, properties);
    const normalized = normalizeCustomEvent(eventName, properties);
    return {
...
export function adaptEvent(eventName, properties) {
    validateEventPayload(eventName, properties);
    const normalized = normalizeCustomEvent(eventName, properties);
    return {
